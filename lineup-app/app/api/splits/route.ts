 ...
 *
 * Deliberately roster-agnostic — just fetches splits for whatever mlbamIds
 * it's given, so it works the same regardless of whose roster called it.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const ids: number[] = mlbamIds.filter((id: number) => id > 0);
    ...
    const [hitterResults, pitcherResults] = await Promise.all([
      Promise.allSettled(
        ids.map(async (mlbamId) => {
          const [platoon, form30, form10] = await Promise.all([
            getPlatoonSplits(mlbamId, season),
            getRecentForm(mlbamId, dateISO, 30),
            getRecentForm(mlbamId, dateISO, 10),
          ]);
          return { mlbamId, platoon, form30, form10 };
        })
      ),
      Promise.allSettled(
    ...
    hitterResults.forEach((result, i) => {
      ...
      if (result.status === "fulfilled") {
        if (result.value.platoon.vsL) vsL[mlbamId] = result.value.platoon.vsL;
        if (result.value.platoon.vsR) vsR[mlbamId] = result.value.platoon.vsR;
        if (result.value.form30) form30[mlbamId] = result.value.form30;
        if (result.value.form10) form10[mlbamId] = result.value.form10;
      } else {
        errors[`player:${mlbamId}`] = String(result.reason?.message ?? result.reason);
      }
    });
